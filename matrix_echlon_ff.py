...
K.<a> = GF(2**3, name='a', modulus=x^3 + x + 1)

# Display all elements of the field
print("Elements of F_8:")
for i, elem in enumerate(K):
    print("{}: {}".format(i, elem))
print()

# Display addition table with polynomial representations
print("Addition table for F_8 (in terms of a where a^3 + a + 1 = 0):")
...
print("-" * 40)
echelon_form = matrix_to_echelon(sample_matrix, show_steps=True)
...
# Function to create custom matrices for testing
def create_test_matrix(rows, cols):
    """Create a random matrix over F_8 for testing"""
    return matrix(K, [[K.random_element() for _ in range(cols)] for _ in range(rows)])

# Example of creating and reducing a custom matrix
print("\n" + "="*50)
print("CUSTOM MATRIX EXAMPLE")
print("="*50)

custom_matrix = create_test_matrix(3, 4)
print("Random 3x4 matrix over F_8:")
print(custom_matrix)
print()

custom_rref = matrix_to_reduced_echelon(custom_matrix, show_steps=False)
print("Its reduced row echelon form:")
print(custom_rref)